{
  ...
  monitoringMetrics = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      done < <(${pkgs.podman}/bin/podman container inspect --format '{{.Image}}' $(${pkgs.podman}/bin/podman container ls --all --quiet) 2>/dev/null || true)
      ...
      while IFS=' ' read -r image_id image_size; do
        ...
        for image_id in $(${pkgs.podman}/bin/podman image ls --quiet --no-trunc 2>/dev/null | ${pkgs.coreutils}/bin/sort --unique); do
          ${pkgs.podman}/bin/podman image inspect --format '{{.Id}} {{.Size}}' "$image_id" 2>/dev/null || true
        done
      )

      printf 'infra_podman_image_count %s\n' "$podman_image_count" >>"$temporary_file"
      printf 'infra_podman_image_size_bytes %s\n' "$podman_image_size_bytes" >>"$temporary_file"
      printf 'infra_podman_reclaimable_image_count %s\n' "$podman_reclaimable_image_count" >>"$temporary_file"
      printf 'infra_podman_reclaimable_image_size_bytes %s\n' "$podman_reclaimable_image_size_bytes" >>"$temporary_file"
      ...
      write_housekeeping_metrics() {
        ...
        if [ -r "$state_file" ]; then
          # The state files are generated by the repository-owned housekeeping scripts.
          source "$state_file"
        fi
