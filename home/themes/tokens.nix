...
let
  ...
  palettesDir =
    ...
    if builtins.pathExists p then
      ...
      throw "hm.theme: palettes directory '${toString p}' does not exist";
  ...
  validate =
    name: value:
    if (value ? name) && (value ? tokens) && (value ? raw) then
      value
    else
      throw "Palette '${name}.nix' must export { name; tokens; raw; }";

  normalized = lib.mapAttrs validate imported;

  # Optional host-level choice: ui.theme.scheme
  hostScheme =
    if (osConfig ? ui) && (osConfig.ui ? theme) && (osConfig.ui.theme ? scheme) then
      osConfig.ui.theme.scheme
    else
      null;
  ...
  chosenPalette =
    if builtins.hasAttr chosenName normalized then
      normalized.${chosenName}
    else
      throw "hm.theme: palette '${chosenName}' not found. Available: ${lib.concatStringsSep ", " available}";
